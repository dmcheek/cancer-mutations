...
setwd("~/HMS Dropbox/Naxerova_Lab/Dave/Causation/to publish/figures/figure 3")
...
ggsave("mean age in AML.pdf",width=5,height=4.5)
...
ggplot(AMLCH_overview,aes(x=AMLy_f,y=AMLo_f,label=Gene))+
 geom_point()+geom_text_repel()+scale_x_continuous(trans="log10",breaks=c(0,0.001,0.003,0.01,0.03,0.1,0.3),labels=c(0,0.001,0.003,0.01,0.03,0.1,0.3))+scale_y_continuous(trans="log10",breaks=c(0,0.01,0.03,0.1,0.3),labels=c(0,0.01,0.03,0.1,0.3))+labs(x="mutation frequency in young-onset AML",y="mutation frequency in adult-onset AML",caption=paste("rho=",signif(unname(sp$estimate),2),", p=",signif(sp$p.value,2),#,"\nr=",signif(unname(p$estimate),2),", p=",signif(p$p.value,2)
                                                                                                                                                                                                                                                                                                                                                                                                           sep=""))+theme_classic()
ggsave("oldmatched_vs_young_freq.pdf",width=5.5,height=4.5)

AMLCH_overview$Gene[AMLCH_overview$AMLo_f<AMLCH_overview$AMLy_f]
...
# cor.test(AMLCH_overview$AMLo_f/AMLCH_overview$L,AMLCH_overview$AMLy_f/AMLCH_overview$L,method="spearman")
# ggplot(AMLCH_overview,aes(x=1000*AMLy_f/L,y=1000*AMLo_f/L,label=Gene))+geom_point()+geom_text_repel()+scale_x_continuous(trans=transx,breaks=c(0,0.001,0.003,0.01,0.03,0.1,0.3),labels=c(0,0.001,0.003,0.01,0.03,0.1,0.3))+scale_y_continuous(trans=transy,breaks=c(0,0.01,0.03,0.1,0.3),labels=c(0,0.01,0.03,0.1,0.3),limits=c(0,0.3))+labs(x="mutations per kilobase per childhood AML sample",y="mutations per kilobase per adult AML sample")+theme_classic()
# ggsave("oldmatched_vs_young_sel.pdf",width=5.5,height=4.5)
...
ggplot(AMLCH_overview,aes(x=ce_y,y=pmin(ce_o,30000),label=Gene))+geom_smooth(data=filter(AMLCH_overview,Gene!="NPM1"),method="lm",se=F,colour=viridis(n=1,begin=0.8,end=0.8,option="mako",alpha=0.5))+geom_point(aes(shape=Gene=="NPM1"),show.legend=F)+geom_text_repel()+scale_x_continuous(trans="log10",breaks=c(0,10,100,1000),labels=c(0,10,100,1000))+scale_y_continuous(trans="log10",breaks=c(0,1,10,100,1000,10000))+
  labs(x="estimated relative carcinogenic effect for young-onset AML",y="estimated carcinogenic effect for adult-onset AML",caption=paste("rho=",signif(unname(sp$estimate),2),", p=",signif(sp$p.value,2),#"\nr=",signif(unname(p$estimate),2),", p=",signif(p$p.value,2),
                                                                                                                      sep=""))+theme_classic()
ggsave("old_vs_young_ce.pdf",width=5.5,height=4.5)
cor.test(AMLCH_overview$ce_y,AMLCH_overview$cellfrac_N/AMLCH_overview$L,method="spearman")
cor.test(AMLCH_overview$ce_y,AMLCH_overview$cellfrac_N,method="spearman")
...
ggsave("AMLmeanagefreq.pdf",width=5.5,height=4.5)
...
#cor.test(log(AMLCH_overview$CH_f/AMLCH_overview$L),AMLCH_overview$AML_mean_age,method="pearson")
...
# ggplot(AMLCH_overview,aes(x=CH_f/L,y=AML_mean_age,label=Gene))+geom_point(aes(shape=signif_sel_normal_blood_text))+geom_text_repel()+scale_shape_manual(values=c(16,8))+scale_x_continuous(trans="log10")+geom_smooth(method="lm",se=F)+
#   labs(y="mean age in AML",x="mutations per kilobase per normal blood cell",shape="dN/dS>1\nin normal\nblood",caption=paste("rho=",signif(unname(sp$estimate),2),", p=",signif(sp$p.value,2),sep=""))+theme_classic()
ggsave("AMLage_CHsel.pdf",width=5.5,height=4.5)

# sp<-cor.test(AMLCH_overview$CH_fpos,AMLCH_overview$AML_mean_age,method="spearman")
# ggplot(AMLCH_overview,aes(x=CH_fpos,y=AML_mean_age,label=Gene))+guides(size="none")+geom_smooth(method="lm",se=F)+geom_point()+geom_text_repel()+scale_x_continuous(trans="log10")+
#   labs(y="mean age in AML",x="mean VAF in normal blood",caption=paste("rho=",signif(unname(sp$estimate),2),", p=",signif(sp$p.value,2),sep=""))+theme_classic()+scale_colour_viridis(direction=-1,option="G",breaks=c(0,1,2),limits=c(0,2),labels=c(0,1,">2"))
# ggsave("AMLage_CHmeanVAF.pdf",width=4.5,height=4)
...
ggsave("AMLage_ce.pdf",width=5.5,height=4.5)
...
ggsave("AMLage_cer.pdf",width=5.5,height=4.5)
...
ggsave("AMLcorrelationsummary.pdf",width=4.5,height=4.5)
...
ggsave("diff rank carcinogenic effect.pdf",width=4.5,height=5)


agedataboxes<-read.csv("~/HMS Dropbox/Naxerova_Lab/Dave/Causation/to publish/data/AMLdataageboxes.csv")
...
ggsave("databoxes.pdf",width=4.5,height=4.5)


cor_bounds<-read.csv("~/HMS Dropbox/Naxerova_Lab/Dave/Causation/to publish/data/AMLrobustness.csv")
...
ggsave("AMLoldyoungrobustness.pdf",width=8,height=3.5)
